...
_HEADERS = {"X-Internal-Api-Key": INTERNAL_SERVICE_API_KEY}


def backend_get(path: str, params: dict | None = None) -> dict:
    resp = httpx.get(f"{BACKEND_URL}{path}", params=params, headers=_HEADERS, timeout=5.0)
    return _handle(resp)


def backend_post(path: str, params: dict | None = None) -> dict:
    resp = httpx.post(f"{BACKEND_URL}{path}", params=params, headers=_HEADERS, timeout=5.0)
    return _handle(resp)
