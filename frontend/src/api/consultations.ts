...
// anything else gets a 400 from the backend.
...
// Google Maps "get directions" deep link — works for both registered vets
// (lat/lng from our own Clinic record) and Google-sourced clinics (lat/lng
// from the Places search response, plus place_id for extra precision).
...
export const fetchNearbyClinics = async (
  ...
  const response = await apiClient.get<NearbyClinicsResponse>('/doctors/nearby-clinics', {
    params: { latitude, longitude, radius_km: radiusKm },
  });
  return response.data;
};
...
export const fetchClinicDetails = async (placeId: string): Promise<PlaceDetailsResponse> => {
  const response = await apiClient.get<PlaceDetailsResponse>(`/doctors/nearby-clinics/${encodeURIComponent(placeId)}`);
  return response.data;
};
